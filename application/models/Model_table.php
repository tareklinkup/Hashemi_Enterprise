...
class Model_Table extends CI_Model
{
    ...
    public function getTransactionSummary($date = null)
    {
        $transactionSummary = $this->db->query("
            ...
            (
                ...
                and ep.status = 'a'
                and ep.payment_type = 'salary_payment'
                " . ($date == null ? "" : " and ep.payment_date < '$date'") . "
            ) as employee_payment,
            (
                select ifnull(sum(el.payment_amount), 0) from tbl_employee_loan el
                where el.branch_id = " . $this->session->userdata('BRANCHid') . "
                and el.status = 'a'
                " . ($date == null ? "" : " and el.payment_date < '$date'") . "
            ) as employee_loan,
            /* total */
            (
            ...
            (
                select paid_purchase + paid_customer + paid_supplier + paid_cash + bank_deposit + employee_payment + employee_loan
            ) as total_out,
            (
    ...
    public function stockValue()
    {
        ...
        $stockValue = $this->db->query("
            select sum(stock_value) as stock_value from(
                select
    ...
    {
        $branchId = $this->session->userdata('BRANCHid');
        $totalCustomerDue = $this->db->query("
        select sum(dueAmount) as total_customer_due from (
            select
            c.Customer_SlNo,
            c.Customer_Name,
            c.Customer_Code,
