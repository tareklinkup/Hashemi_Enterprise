...
class Products extends CI_Controller
{
    ...
    public function getProductsBySupplier()
    {
        ...
        $products = $this->db->query("
            ...
            from tbl_product p
            left join tbl_productcategory pc on pc.ProductCategory_SlNo = p.ProductCategory_ID
            left join tbl_purchasedetails pd on pd.Product_IDNo = p.Product_SlNo
            ...
            where p.status = 'a'
            and pm.Supplier_SlNo = '$data->supplierId'
            and pd.Product_IDNo = p.Product_SlNo
            $clauses
            order by p.Product_SlNo desc
        ")->result();
...
<?php
    ...
    public function getProductLedger()
    {
        ...
        foreach ($ledger as $key => $value) {
            if ($value->date >= $data->dateFrom && $value->date <= $data->dateTo) {
                array_push($content, $value);
            }
        }

        echo json_encode(['ledger' => $content, 'previousStock' => $previousStock]);
        // echo json_encode(['ledger' => $ledger]);
    }
}