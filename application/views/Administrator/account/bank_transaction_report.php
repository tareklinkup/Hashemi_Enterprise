                ...
                <table class="table table-bordered table-condensed" id="transactionsTable">
                    ...
                    <tbody>
                        ...
                        <tr v-for="(transaction, sl) in transactions">
                            ...
                            <td style="text-align:right">{{ transaction.deposit_amount }}</td>
                            <td style="text-align:right">{{ transaction.withdraw_amount }}</td>
                    ...
                    <tfoot style="display: none">
                        <tr style="font-weight:bold;">
                            ...
                            <td style="text-align:right;">{{ totalDeposit = this.transactions.reduce((prev, curr) => { return prev + parseFloat(curr.deposit_amount)}, 0) }}</td>
                            <td style="text-align:right;">{{ totalWithdraw = this.transactions.reduce((prev, curr) => { return prev + parseFloat(curr.withdraw_amount)}, 0) }}</td>
                            <td style="text-align:right;">{{ totalBalance = this.transactions.reduce((prev, curr) => { return prev + parseFloat(curr.balance)}, 0) }}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
    ...
    new Vue({
        ...
        methods: {
            ...
            getTransactions(){
                ...
                axios.post('/get_bank_transactions_ledger', this.filter)
                .then(res => {
                    this.transactions = res.data.payments;
