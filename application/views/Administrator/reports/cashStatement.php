		...
		<div class="row" style="padding-top:10px;">
			...
			<div class="col-md-6">
				...
				<table class="table table-bordered table-condensed">
					...
					<tbody style="display:none;" v-bind:style="{display: employeeLoan.length > 0 ? '' : 'none'}">
						<tr v-for="loan in employeeLoan">
							<td>{{ loan.Employee_ID }}</td>
							<td>{{ loan.Employee_Name }}</td>
							<td>{{ loan.payment_date }}</td>
							<td style="text-align:right;">{{ loan.payment_amount | decimal }}</td>
						</tr>
					</tbody>
					<tfoot>
						<tr style="font-weight:bold;">
							<td colspan="3" style="text-align:right">Total</td>
							<td style="text-align:right;">
								<span v-if="employeeLoan.length == 0">0.00</span>
								<span style="display:none;" v-bind:style="{display: employeeLoan.length > 0 ? '' : 'none'}">{{ totalEmployeeLoan | decimal }}</span>
							</td>
						</tr>
					</tfoot>
				</table>
...
<script>
	new Vue({
		...
		computed: {
			...
			totalEmployeeLoan() {
				return this.employeeLoan.reduce((prev, curr) => {
					return prev + parseFloat(curr.payment_amount)
				}, 0).toFixed(2);
			},
			totalCashIn() {
			...
			totalCashOut() {
				return parseFloat(this.totalPurchase) +
					...
					parseFloat(this.totalEmployeeLoan);
			},
			cashBalance() {
		...
		methods: {
			...
			getEmployeeLoan() {
				axios.post('/get_employee_loan', this.filter)
					.then(res => {
						this.employeeLoan = res.data;
					})
			},
