              ...
              <div class="row">
                ...
                <div class="col-sm-4">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-7">
                      <select onchange="onChangeCustomer(this)" class="chosen-select form-control" id="cus_id" required name="cus_id" style="height: 30px; border-radius: 5px;">
...
<script>
let id = document.getElementById("cus_id");
let selectedCustomer = id.options[id.selectedIndex].value;

function onChangeCustomer(customerId) {
  let selectedValue = customerId == undefined ? selectedCustomer : customerId.value;
  axios.post("/get_customers", {customerId: selectedValue}).then(res => {
    let r = res.data[0];
    if(r != undefined){
      let input = ` 
      <div class="form-group">
        <label class="col-sm-5 control-label no-padding-left" for="bank_name">Customer Address:<span class="text-bold text-danger">*</span> </label>
        <div class="col-sm-7">
          <input type="text" class="form-control" id="customer_address" value="${r.Customer_Address}"  />
        </div>
      </div>`;
      document.getElementById("customerAddress").innerHTML = input;
    }else{
