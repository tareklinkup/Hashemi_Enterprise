...
<div id="salaryReport">
	<div class="row" style="border-bottom:1px solid #ccc;padding: 10px 0;">
		<div class="col-md-12">
			<form class="form-inline" @submit.prevent="getLoanLedger">
	...
	<div class="row" style="margin-top: 10px;display:none;" v-bind:style="{display: loanLedger.length > 0 || previousBalance != 0  ? '' : 'none'}">
				...
				<div>
					...
					<table class="table table-bordered table-condensed">
						...
						<tbody>
							<tr>
								<td>0</td>
								<td colspan="5" style="text-align: left;font-weight:bold">Previous Balance</td>
								<td style="text-align:right;">{{ previousBalance }}</td>
							</tr>
							<tr v-for="(loan, sl) in loanLedger">
								<td>{{ sl + 1 }}</td>
								...
								<!-- <td>{{ loan.month_name }}</td> -->
								<td style="text-align:right;">{{ loan.loan_taken }}</td>
								<td style="text-align:right;">{{ loan.loan_paid }}</td>
								<td style="text-align:right;">{{ loan.balance }}</td>
							</tr>
						</tbody>
						<tfoot>
							<tr style="font-weight:bold;" v-if="loanLedger.length > 0">
								...
								<!-- <td style="text-align:right;">{{ (payments.reduce((prev, curr) => { return prev + parseFloat(curr.salary_range)}, 0) - payments.reduce((prev, curr) => {return prev + parseFloat(curr.payment_amount) + parseFloat(curr.deduction_amount)}, 0)).toFixed(2) }}</td> -->
								<!-- <td style="text-align:right;">
									{{ parseFloat(payments[payments.length-1].balance).toFixed(2)}}
								</td> -->
							</tr>
						</tfoot>
					</table>
	...
	new Vue({
		...
		methods: {
			...
			getLoanLedger() {
				...
				axios.post('/get_employee_loan_ledger', data).then(res => {
					// console.log(res);
					this.loanLedger = [];
					this.previousBalance = 0;
					let sum = 0;
					res.data.forEach(element => {
						sum += parseFloat(element.loan_taken) - parseFloat(element.loan_paid);

						// if (element.date >= this.dateFrom && element.date <= this.dateTo) {

						if (element.date >= this.dateFrom && element.date <= this.dateTo) {
							element.balance = sum;
							this.loanLedger.push(element);
						} else if (element.date < this.dateFrom) {
							this.previousBalance = sum;
						}
					});
