		...
		<div class="row" style="margin-top: 10px;margin-bottom:15px;border-bottom: 1px solid #ccc;padding-bottom: 15px;">
			<div class="col-md-6 col-md-offset-3">
				<div class="form-group">
					...
					<div class="col-md-7">
						<input type="date" class="form-control" v-model="payment.payment_date">
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-md-7">
						...
						<v-select v-bind:options="employees" label="display_name" v-model="selectedEmployee" style="display:none;" v-bind:style="{display: employees.length > 0 ? '' : 'none'}"></v-select>
					</div>
						...
						<tr>
							...
							<td>
								<?php if ($this->session->userdata('accountType') != 'u') { ?>
									<button type="button" class="button edit" @click="editloan(row)">
										<i class="fa fa-pencil"></i>
									</button>
									<button type="button" class="button" @click="deleteLoanPayment(row.id)">
...
<script>
	...
	new Vue({
		...
		methods: {
			...
			// },
			getPayments() {
				axios.get('/get_employee_loan').then(res => {
			...
			savePayment() {
				...
				// if (this.payable_amount < this.payment.payment_amount) {
				...
				let url = '/add_employee_loan';
				if (this.payment.id != null) {
					url = '/update_employee_loan';
				}
			...
			deleteLoanPayment(Id) {
				...
				axios.post('/delete_employee_loan_payment', {
					id: Id
				}).then(res => {
					let r = res.data;
					alert(r.message);
