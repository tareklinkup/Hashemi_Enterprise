	...
	<div class="row" style="border-bottom: 1px solid #ccc;padding: 3px 0;">
		<div class="col-md-12">
			<form class="form-inline" id="searchForm" @submit.prevent="getSearchResult">
				...
				<div class="form-group" style="display:none;" v-bind:style="{display: (searchType == 'customer' || searchType == 'quantity') && customers.length > 0 ? '' : 'none'}">
				...
				<table 
					...
					<tbody>
						<tr v-for="sale in sales">
							...
							<td style="text-align:right;">{{ sale.SaleMaster_Description }}</td>
							<td style="text-align:center;">
								<a href="" title="Sale Invoice" v-bind:href="`/sale_invoice_print/${sale.SaleMaster_SlNo}`" target="_blank"><i class="fa fa-file"></i></a>
								<a href="" title="Chalan" v-bind:href="`/chalan/${sale.SaleMaster_SlNo}`" target="_blank"><i class="fa fa-file-o"></i></a>
								<?php if($this->session->userdata('accountType') != 'u'){?>
...
<script>
	...
	new Vue({
		...
		methods: {
			...
			getSearchResult(){
				if(this.searchType != 'customer' && this.searchType !="quantity"){
